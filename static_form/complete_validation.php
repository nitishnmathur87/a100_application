...
if (isset($_POST['save'])) 
{
  ...
  $work_authorization = (isset($_POST['residency_status']) ? (string) $_POST['residency_status'] : "yes");
  $path_id = $_POST['path_id'];
  $other_path = $_POST['other_path'];
  $hours = $_POST['hours'];
  $commitments = $_POST['commitments'];
  $program_id = $_POST['program_id'];
  $tech_other = $_POST['list_commit'];
  $work_id = $_POST['work_id'];
  $job_title = $_POST['job_title'];
  $front_end = $_POST['front_end'];
  $app_stack = $_POST['application_stack'];
  $mobile_app = $_POST['mobile_app'];
  $cms = $_POST['cms'];
  $other_rel = $_POST['other_rel'];
  $resume = $_POST['resume'];
  $cover_letter = $_POST['cover_letter'];
  $references = $_POST['references_field'];
  ...
  //check if first name empty
   ...
   // $salt = crypt($password, $email_address);
    ...
    if (empty($_SESSION['personal'])) 
    {
           $result = $db->prepare("INSERT INTO apprentice "
                    . "(first_name, last_name, email_address, password, institution_id, major, graduation_date, cohort_id, address, city, state_id, zip,  phone, "
                   . "linkedin, portfolio, age, residency_status, path_id, other_path) "
            ...
            $result->bindParam(':linkedin', $dbedin, PDO::PARAM_STR);
      ...
      //after the initial data is executed, we get the lastInsertId for the path_db table
    //  $pathStmt = $db->prepare("INSERT INTO path_db(path_id, apprentice_id) VALUES(:path_id, :apprentice_id)");
   //   $pathStmt->bindParam(':path_id', $path_to_a100);
     ...
     // foreach ($_POST['path_to_a100'] as $path_to_a100)     
      ...
      $supportStmt = $db->prepare("INSERT INTO supporting_materials(resume, cover_letter, other, references_field) VALUES(:resume, :cover_letter, :other_support, :references_field)");
      ...
      $supportStmt->bindParam(':references', $references_field);
      $supportStmt->execute();
      $supportId = $db->lastInsertId();
      $supportUpdate = $db->prepare("UPDATE apprentice SET support_id = :support_id WHERE apprentice_id = :insertId");
      ...
      $techStmt = $db->prepare("INSERT INTO technical_experience(other, job_title) VALUES(:other_rel, :job_title)");
      $techStmt->bindParam(':other_rel', $other_rel);
      $techStmt->bindParam('job_title', $job_title);
      $techStmt->execute();
      $techId = $db->lastInsertId();
      $techUpdate = $db->prepare("UPDATE apprentice SET tech_id = :tech_id WHERE apprentice_id = :insertId");
      $techUpdate->bindValue(':tech_id', $techId, PDO::PARAM_INT);
      $techUpdate->bindValue(':insertId', $insertId, PDO::PARAM_INT);
      $techUpdate->execute();
      
      //front-end
      $frontendStmt = $db->prepare("INSERT INTO front_end_language(description) VALUES(:description)");
      $frontendStmt->bindParam(':description', $front_end);
      $frontendStmt->execute();
      $frontendId = $db->lastInsertId();
      $frontendUpdate = $db->prepare("UPDATE technical_experience SET front_end_id = :front_end WHERE tech_id = :techId");
      ...
      //application stack
      $appstackStmt = $db->prepare("INSERT INTO application_stack(description) VALUES(:description)");
      $appstackStmt->bindParam(':description', $app_stack);
      $appstackStmt->execute();
      $appstackId = $db->lastInsertId();
      $appstackUpdate = $db->prepare("UPDATE technical_experience SET app_stack_id = :app_stack WHERE tech_id = :techId");
      ...
      $mobileappStmt = $db->prepare("INSERT INTO mobile_app_develop(description) VALUES(:description)");
      $mobileappStmt->bindParam(':description', $mobile_app);
      $mobileappStmt->execute();
      $mobileappId = $db->lastInsertId();
      $mobileappUpdate = $db->prepare("UPDATE technical_experience SET mobile_app_id = :mobile_app WHERE tech_id = :techId");
      ...
      $cmsStmt = $db->prepare("INSERT INTO content_management_system(description) VALUES(:description)");
      $cmsStmt->bindParam(':description', $app_stack);
      $cmsStmt->execute();
      $cmsId = $db->lastInsertId();
      $cmsUpdate = $db->prepare("UPDATE technical_experience SET content_mgmt_id = :cms WHERE tech_id = :techId");
      $cmsUpdate -> bindValue(':cms', $cmsId, PDO::PARAM_INT);
      $cmsUpdate -> bindValue(':techId', $techId, PDO::PARAM_INT);
      $cmsUpdate -> execute();

      //work experience
      $workStmt = $db->prepare("INSERT INTO technical_experience(work_id) VALUES(:work)");
      $workStmt->bindParam(':work', $work_experience);
      $workStmt->execute();
      
      //programming experience
      $programStmt = $db->prepare("INSERT INTO technical_experience(program_id) VALUES(:program)");
...
if (isset($_POST['submit'])) 
{
  ...
  //check if first name empty{
      ...
      if (empty($dbedin))
      {
        $_SESSION['personal']['dbedin'] = "Please enter your dbedin URL";
      } 
      if (empty($hours))
      {
      ...
      if (empty($_SESSION['personal'])) 
      {
        $result_new = $db->prepare("INSERT INTO apprentice (first_name, last_name, email_address, password, cohort_id, major, graduation_date, address, city, state, zip_code, institution_id, mobile_phone, dbedin_profile, online_portfolio, other, age, work_status, hours, commitments) VALUES (:first_name, :last_name, :email_address, :password, :cohort, :major, :graduation_date, :address, :city, :state, :zip_code, :university_id, :mobile_phone, :dbedin_profile, :online_portfolio, :other, :age, :work_authorization, :hours, :commitments)");
        ...
        $result_new -> bindParam(':dbedin_profile', $dbedin, PDO::PARAM_STR);
        ...
        $complete = 1;
        $completeUpdate = $db->prepare("UPDATE apprentice SET completed = :complete WHERE apprentice_id=:apprentice_id");
